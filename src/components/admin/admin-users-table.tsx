...
const statusGroups: Array<{ description: string; label: string; openWhenFilled?: boolean; status: AccessStatus }> = [
  {
        ...
        {statusGroups.map((group) => {
          ...
          return (
            <details
              ...
              className="group bg-[rgba(24,21,18,0.32)]"
              key={group.status}
              open={Boolean(group.openWhenFilled && groupUsers.length > 0)}
            >
              <summary className="flex cursor-pointer list-none items-center justify-between gap-3 border-b border-[color:var(--color-card-border)] bg-[rgba(245,180,72,0.06)] px-5 py-4 marker:hidden sm:px-6">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="text-sm font-medium text-[color:var(--color-cream)]">{group.label}</h3>
                    <span className="rounded-full bg-[rgba(245,180,72,0.12)] px-2.5 py-1 text-xs font-medium text-[color:var(--color-gold)]">
                      {groupUsers.length}
                    </span>
                  </div>
                  <p className="mt-1 text-xs leading-5 text-[color:var(--color-text-muted)]">{group.description}</p>
                </div>
                <ChevronDown className="h-5 w-5 shrink-0 text-[color:var(--color-gold)] transition group-open:rotate-180" aria-hidden="true" />
              </summary>

              <div className="divide-y divide-[color:var(--color-card-border)]">
                {groupUsers.length > 0 ? (
                  groupUsers.map((user) => <AdminUserRow key={user.id} user={user} />)
                ) : (
  ...
  return (
    <article className="grid gap-4 p-5 sm:grid-cols-[1.4fr_0.9fr_auto] sm:items-center sm:p-6">
      ...
      <div className="flex flex-wrap gap-2 sm:justify-end">
        ...
        {user.status === "suspended" ? <AdminDeleteUserForm userId={user.id} /> : null}
      </div>
    </article>
  );
}
