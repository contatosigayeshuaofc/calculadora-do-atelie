...
const readinessItems = [
  ...
  },
  {
    ...
    description: "Entrar com o e-mail administrador e confirmar que somente esse acesso ve o painel de aprovacao.",
  },
  {
...
export function PilotReadinessCard() {
  return (
    <section className="atelier-panel mt-6 overflow-hidden">
      <div className="border-b border-[color:var(--color-clay-beige)] p-5 sm:flex sm:items-start sm:justify-between sm:gap-4 sm:p-6">
        <div>
          <p className="text-xs font-bold uppercase tracking-[0.16em] text-[color:var(--color-muted-lavender)]">Liberacao do MVP</p>
      ...
      <div className="grid gap-0 divide-y divide-[color:var(--color-clay-beige)] lg:grid-cols-2 lg:divide-x lg:divide-y-0">
        {readinessItems.map((item) => (
          <article className="flex gap-3 p-5 sm:p-6" key={item.title}>
            ...
            <div>
              <h3 className="text-sm font-bold text-[color:var(--color-warm-graphite)]">{item.title}</h3>
      ...
      <div className="flex flex-col gap-3 border-t border-[color:var(--color-clay-beige)] bg-[rgba(248,246,241,0.65)] p-5 sm:flex-row sm:items-center sm:justify-between sm:p-6">
        ...
        <span className="inline-flex items-center gap-2 text-sm font-bold text-[color:var(--color-olive)]">
          Checklist interno
