    ...
    <div className="space-y-5">
      ...
      <section className="rounded-[var(--radius-sm)] border border-[color:var(--color-card-border)] bg-[color:var(--color-card)] p-4 shadow-[var(--shadow-floating)]">
        ...
        <div className="mt-4 grid gap-3 text-sm md:grid-cols-4">
          ...
          <Info label="Pagamento" value={sale.payment_method ?? "Não informado"} />
        ...
        <div className="mt-4 divide-y divide-[color:var(--color-card-border)]">
          {sale.sale_items.map((item) => (
            <div className="grid gap-2 py-3 text-sm md:grid-cols-[1fr_110px_130px_130px]" key={item.id}>
              ...
              <Info label="Unitário" value={formatCurrency(item.unit_price_cents)} />
      ...
      {sale.notes ? (
        <section className="rounded-[var(--radius-sm)] border border-[color:var(--color-card-border)] bg-[color:var(--color-card)] p-4 text-sm text-[color:var(--color-text-muted)] shadow-[var(--shadow-floating)]">
          {sale.notes}
        </section>
      ) : null}
    </div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[var(--radius-sm)] border border-[color:var(--color-card-border)] bg-[color:var(--color-card)] p-4 shadow-[var(--shadow-floating)]">
      ...
      <p className="mt-1 text-lg font-bold text-[color:var(--color-cream)]">
        {value}
      </p>
    </div>
  );
