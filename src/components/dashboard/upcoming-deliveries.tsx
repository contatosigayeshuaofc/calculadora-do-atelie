...
export function UpcomingDeliveries({ currencyCode, deliveries }: UpcomingDeliveriesProps) {
  return (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="mt-1 text-sm text-[color:var(--color-text-muted)]">
                  {formatDate(sale.deliveryDate)} · {formatCurrency(sale.totalCents, currencyCode)}
                </p>
              </div>
              {sale.id ? (
                <div className="flex gap-2 sm:justify-end">
                  <Link href={`/vendas/${sale.id}` as never}>
                    <Button aria-label="Visualizar entrega" leftIcon={<Eye className="h-4 w-4" />} size="icon" variant="secondary" />
                  </Link>
                  <Link href={`/vendas/${sale.id}/editar` as never}>
                    <Button aria-label="Editar entrega" leftIcon={<Edit3 className="h-4 w-4" />} size="icon" variant="ghost" />
                  </Link>
                </div>
              ) : null}
            </article>
