      ...
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-col gap-4 border-b border-[color:var(--color-card-border)] pb-6 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="text-xs font-normal uppercase tracking-[0.18em] text-[color:var(--color-antique-gold)]">Administração</p>
              <h1 className="mt-1 font-medium text-4xl leading-none">Acessos das clientes</h1>
...
function Metric({ label, value }: { label: string; value: number }) {
  return (
    <div className="atelier-panel p-5">
      <p className="text-xs font-normal uppercase tracking-[0.14em] text-[color:var(--color-muted-lavender)]">{label}</p>
      <p className="mt-2 font-medium text-4xl leading-none text-[color:var(--color-cream)]">{value}</p>
    </div>
  );
}
